...
**Commands:** `npm run dev` / `build` / `start` / `lint`; `npm run db:generate` / `db:migrate` / `db:push` / `db:studio` (Drizzle Kit); `npm run pw:run` (all Playwright e2e in `tests/e2e/`, reuses the dev server on :3000) or `npx playwright test tests/e2e/<feature>.spec.ts` for one feature. Playwright is the only maintained e2e suite: write new specs there. `cypress/` is kept for reference but no longer updated. Node 24 via `.nvmrc`.
